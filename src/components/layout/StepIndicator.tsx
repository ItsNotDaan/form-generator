...
export const StepIndicator = React.memo<StepIndicatorProps>(({ currentStep, onBackButtonClicked }) => {
      ...
      <div className="items-center hidden gap-2 md:flex">
        {steps.map((step, index) => (
          <React.Fragment key={step.number}>
            <div
              className={`flex items-center gap-2 ${step.number < currentStep ? 'cursor-pointer' : ''
                }`}
              onClick={() => handleStepClick(step)}
            >
              <div
                className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold transition-all ${step.number === currentStep
              ...
              <span
                className={`text-sm font-medium hidden lg:inline ${step.number === currentStep
                  ? 'text-white'
                  : step.number < currentStep
                    ? 'text-white/80'
                    : 'text-white/50'
            ...
            {index < steps.length - 1 && (
              <div
                className={`h-0.5 w-12 ${step.number < currentStep ? 'bg-white/50' : 'bg-white/20'
                  }`}
              />
            )}
          </React.Fragment>
