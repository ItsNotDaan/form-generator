...
export const Alert = React.memo<AlertProps>(
  ({status = 'info', children, className = ''}) => {
    const baseStyles =
      'flex gap-3 p-4 rounded-1.5 border';

    const statusStyles = {
      info: 'bg-blue-100 border-blue-400 text-blue-700',
      warning: 'bg-orange-200 border-orange-500 text-orange-700',
...
export const AlertIcon = React.memo<{status?: 'info' | 'warning' | 'success' | 'error'}>(
  ({status = 'info'}) => {
    const icons = {
      info: (
    ...
    return <div className="flex-shrink-0">{icons[status]}</div>;
  }
);
