...
import { Zijde, PULMAN_TYPE_OPTIES, SCHOENMATEN, PAARTYPE_OPTIES } from '@/lib/constants/formConstants';
        ...
        <BaseLayout title={t('intakePulman')} currentStep={2}>
            <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex flex-col items-center justify-center gap-3 mb-12">
                    <h1 className="text-4xl font-bold text-foreground">{t('intakePulman')}</h1>
                    <p className="text-lg text-muted-foreground">{t('pulmanDescription')}</p>
                </div>
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit, scrollToFirstError)} className="space-y-6">
                            <Card>
                                ...
                                <CardContent>
                                    <FormField control={form.control} name="welkPaar" render={({ field }) => (
                                        <FormItem>
                                            <FormControl>
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Select which side (left, right, or both)</CardDescription>
                                </CardHeader>
                                <CardContent>
                                                    ...
                                                    <div className="flex gap-6">
                                                        {['both', 'left', 'right'].map(s => (
                                                            <div key={s} className="flex items-center space-x-2">
                                                                ...
                                                                <Label htmlFor={`side-${s}`} className="font-normal cursor-pointer">{t(s)}</Label>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </RadioGroup>
                            ...
                            <Card>
                                <CardHeader><CardTitle>{t('medicalIndication')}</CardTitle></CardHeader>
                                <CardContent>
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Is the affected area bandaged?</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <FormField control={form.control} name="gezwachteld" render={({ field }) => (
                                        <FormItem>
                                            <FormControl>
                                                <RadioGroup onValueChange={(v) => field.onChange(v === 'yes')} value={field.value ? 'yes' : 'no'}>
                                                    <div className="flex gap-6">
                                                        {['yes', 'no'].map(v => (
                                                            <div key={v} className="flex items-center space-x-2">
                                                                <RadioGroupItem value={v} id={`bandaged-${v}`} />
                                                                <Label htmlFor={`bandaged-${v}`} className="font-normal cursor-pointer">{t(v)}</Label>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </RadioGroup>
                            ...
                            <Card>
                                <CardHeader><CardTitle>{t('pulmanType')}</CardTitle></CardHeader>
                                <CardContent>
                                    <FormField control={form.control} name="typePulman" render={({ field }) => (
                                        <FormItem>
                                            <Select onValueChange={field.onChange} value={field.value} disabled={gezwachteld}>
                                                <FormControl>
                                                    <SelectTrigger>
                            ...
                            <Card>
                                <CardHeader><CardTitle>{t('shoeSize')}</CardTitle></CardHeader>
                                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <FormField control={form.control} name="schoenmaat" render={({ field }) => (
                                        <FormItem>
                                    ...
                                    <FormField control={form.control} name="afgegevenMaat" render={({ field }) => (
                                        <FormItem>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <FormField control={form.control} name="bijzonderheden" render={({ field }) => (
                                        <FormItem>
                                            <FormControl>
                                                <Textarea placeholder={t('specialNotesPlaceholder')} rows={5} className="resize-none" {...field} />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )} />
