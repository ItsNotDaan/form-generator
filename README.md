...
- **Single source of truth**: Alle form types zijn gedefinieerd in `src/backend/types/formData.ts`
...
- Het build-time script `scripts/generate-empty-data.ts` gebruikt de TypeScript Compiler API om:
  ...
  - Automatisch empty object templates te genereren in `src/backend/types/formDataTemplates.generated.ts`
